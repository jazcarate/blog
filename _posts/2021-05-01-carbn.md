...
I find it curious that things like a "Natural Language Processing" (NLP) exist to bridge the gap between humans and computer.
...
In it's purest form, we have myriad of abstractions piled up to get from:
...
Even though it is a bit more verbose, it is an order of magnitude faster than the other two flavours. We'll see why in the next section.

## Software's humble beginnings

Let's start with a review of how we got here.

In the beginning, human wrote *machine code*. A set of instructions that a computer would execute. We figured out that this was very powerful, but also very easy to make mistakes. We spend a lot of time thinking, and not a lot ot time writing programs; as the economic balance was tipped in the machine's favour.

By the hard work of many people, we started to be able to have more widespread computational access, cheaper.
...
```
And this is toy example, to keep things short. We hadn't dealt with dynamic arrays, we know that we wanted the third position, so we could do some pre-emptive calculations on our head before writing this.
...
What if we actually wanted the second item of a list, rather than the third? We didn't have such compiler... that is, **until now**.
...
We fully understand the problem to solve. After all, out team was taught in the classic silicon-transistor-Von Neumann way of thinking about software and code. We want more expression, and safety in the code we write; but we also want code that humans can understand and reason about.

We are stretching the problem space of the code in two very different directions:

1. On the one hand we want to eventually get our code to get transformed in machine instructions
...
This fires up two process:
One that quickly returns a UUID of the computation. This is like a cubbyhole for your result.
And the other start the orchestration, parsing, distribution and eventual reduction of the result, which will at some point compute:
```md
25
```
...
As soo as they can type an answer, the cubyhole will get filled.

## Pricing
You will see in out pricing strategy that we are order of magnitude more expensive than other cloud computing providers, we don't have nearly as much up uptime as them, and our latency is much much higher, but can they:
...
Time is cheap. Correctness and perfection is what you would be paying for with `carbn`.
...
The initial payload is first split in chunks for better parallelization and scoping.
We can't reasonably give all the program to just one _agent_, so we split each stack so that they can work in the smallest, complete definition of a sub-problem. So you can structure your code in such a way to limit exposure to secrets from inner agents.
...
#### What is doing the orchestration, filling cubyholes, responding though HTTP, et al.?
It is true that we are using outdated technologies to do this plumbing.
We are working very hard to bootstrap YYY so itself can run on YYY. For the time being, it is cheaper and more cost effective to use unfashionable code. 
